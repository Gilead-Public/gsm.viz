...
describe("points/validateSpec", () => {
  ...
  describe("scales", () => {
    test.each([
      ...
      expect(() => validateSpec(data, { ...minimalSpec, scales })).toThrow(
        message
      );
    });

    test.each(["x", "y"])("validates the %s scale independently", (axis) => {
      expect(() =>
        validateSpec(data, {
          ...minimalSpec,
          scales: { [axis]: [] },
  ...
  describe("categorical color", () => {
    ...
    test.each([
      ["non-object scale", [], "spec.scales.color must be a plain object"],
      [
        "unsupported field",
        { unknown: true },
  ...
  describe("callbacks", () => {
    test.each([null, [], "invalid"])(
      ...
      (callbacks) => {
        expect(() => validateSpec(data, { ...minimalSpec, callbacks })).toThrow(
  ...
  describe("selection", () => {
    test.each([null, [], "invalid"])(
      ...
      (selection) => {
        expect(() => validateSpec(data, { ...minimalSpec, selection })).toThrow(
  ...
  describe("theme", () => {
    test.each([null, [], "invalid"])(
      ...
      (theme) => {
        expect(() => validateSpec(data, { ...minimalSpec, theme })).toThrow(
          "spec.theme must be a plain object"
        );
      }
    );
