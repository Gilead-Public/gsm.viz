fetch('data/ae.json')
    ...
    .then((json) => {
        ...
        const subjectToSite = new Map(
            allSubjects.map((s) => [s, s.split('-')[1]])
        );
        ...
        function buildPrevalenceData(variable, site, subject) {
            const studyEnabled =
                document.getElementById('ae-level-study').checked;
            const siteEnabled =
                document.getElementById('ae-level-site').checked;
        ...
        // Level checkboxes
        ['ae-level-study', 'ae-level-site', 'ae-level-subject'].forEach(
            (id) => {
